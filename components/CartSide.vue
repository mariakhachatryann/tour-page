<template>
    <div class="bg-white h-screen md:w-[312px] w-[512px] md:overflow-y-scroll" :class="{'overflow-y-scroll': myCart.transfers.length > 1}">
        <div class="flex">
            <div class="w-full py-3 px-4 flex justify-between bg-[#E8F0F9]">
                ...
                <div class="flex gap-2 items-center">
                    ...
                    <div class="rounded-2xl h-6 w-6 items-center bg-[#088C53] text-sm font-medium p-2 border-[1px] border-solid border-white text-[#FDFFFE] flex justify-center">{{ myCart.transfers.length }}</div>
                </div>
            </div>
        ...
        <div v-if="myCart.transfers.length === 0" class="h-full w-full flex justify-center items-center flex-col gap-2">
            <div>
        ...
        <div class="w-full flex justify-center items-center mt-10" v-else>
            <div class="max-w-[1384px] w-full mx-4 p-4">
                ...
                <div class="border-b-[1px] border-x-[1px] rounded-b-2xl border-solid border-[#D0E1F3]">
                    <TransferCart :font-size="'14px'" v-for="transfer in myCart.transfers" :key="transfer" :transfer="transfer" />
                </div>
            </div>
        </div>
    </div>
