<template>
    <div class="bg-white h-screen md:w-[312px] w-[512px] md:overflow-y-scroll" :class="{'overflow-y-scroll': transfers.length > 1}">
        <div class="flex">
            <div class="w-full py-3 px-4 flex justify-between bg-[#E8F0F9]">
                ...
                <div class="flex gap-2 items-center">
                    ...
                    <div class="rounded-2xl h-6 w-6 items-center bg-[#088C53] text-sm font-medium p-2 border-[1px] border-solid border-white text-[#FDFFFE] flex justify-center">{{ transfers.length }}</div>
                </div>
            </div>
        ...
        <div v-if="!transfers.length " class="h-full w-full flex justify-center items-center flex-col gap-2">
            <div>
...
onMounted(() => {
    if (transfersData === null) {
        transfers = []; // Initialize as an empty array when there's no data in localStorage
    } else {
