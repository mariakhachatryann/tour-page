<template>
    <div class="flex justify-evenly gap-14 h-[80px] bg-white border-b-[1px] border-solid border-[#E6E7E8] pt-2 xl:justify-between px-8 sm:gap-2 sm:px-1 sm:justify-between">
        ...
        <div class="mt-2 w-[470px] sm:hidden lg:hidden flex items-center" :class="{ ' lg:hidden': !menu}">
            <ul class="menu flex justify-start sm:hidden items-center">
                <NuxtLink @click="closeMenu" to="/tours" class="line-clamp-1 text-[16px] text-primaryGrey p-2 mr-2 cursor-pointer hover:text-primaryBlue hover:duration-300">Tours</NuxtLink>
                <NuxtLink @click="closeMenu" to="/hotels" class="line-clamp-1 text-[16px] text-primaryGrey p-2 mr-2 cursor-pointer hover:text-primaryBlue hover:duration-300">Hotels</NuxtLink>
                <NuxtLink @click="closeMenu" to="/transfer" class="line-clamp-1 text-[16px] text-primaryGrey p-2 mr-2 cursor-pointer hover:text-primaryBlue hover:duration-300">Transfer</NuxtLink>
                <NuxtLink @click="closeMenu" to="/outgoings"  class="line-clamp-1 text-[16px] text-primaryGrey p-2 mr-2 cursor-pointer hover:text-primaryBlue hover:duration-300">Outgoing tours</NuxtLink>
                <NuxtLink @click="closeMenu" to="/contact" class="line-clamp-1 text-[16px] text-primaryGrey p-2 mr-2 cursor-pointer hover:text-primaryBlue hover:duration-300">Contact us</NuxtLink>
            </ul>
        </div>
        <div class="flex flex-row md:justify-between items-center gap-4">
            ...
            <div class="cursor-pointer">             
                <svg v-if="!menu" @click="toggleMenu" class="m-2 mt-5 ml-2 hidden lg:block" :class="{  'sm:hidden': menu }" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <div v-else>                    
                    <svg @click="toggleMenu" class="m-2 mt-5 ml-2 hidden sm:block lg:block" :class="{ 'sm:hidden md:hidden': !menu }" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
    ...
    <div class="hidden z-10 bg-white absolute w-full pb-8" :class="{ 'sm:block lg:block': menu}">
        <div>
